...
contract LootLib {
    ...
    function getElements(uint256 tokenId) 
        internal
        view
        returns (
            string[8] memory _elements 
            
        ) {
            _elements[0] = getWeapon(tokenId);
            _elements[1] = getChest(tokenId);
            _elements[2] = getHead(tokenId);
            _elements[3] = getWaist(tokenId);
            _elements[4] = getFoot(tokenId);
            _elements[5] = getHand(tokenId);
            _elements[6] = getNeck(tokenId);
            _elements[7] = getRing(tokenId);
            return _elements;
    }

    function tokenURI(uint256 tokenId) public view returns (string memory) {
        
        string[8] memory elements = getElements(tokenId);

        return json.formattedMetadata(
            string.concat('Bag #', utils.toString(tokenId)),
...
contract LootOriginal {
    ...
    function tokenURI(uint256 tokenId) public view returns (string memory) {
        ...
        output = string(abi.encodePacked('data:application/json;base64,', _json));
